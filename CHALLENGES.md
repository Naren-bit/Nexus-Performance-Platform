...
- **AI Integration & Security (Claude AI & Copilot):** Storing private Anthropic/OpenAI API keys in a client-side React frontend exposes keys to the public. For the hackathon, we simulated the Admin insights dynamically while building an interactive **AI Goal Copilot** on the employee goal creation page that utilizes standard Thrust Area mappings to draft optimized, high-fidelity SMART goals. A production path would utilize a secure backend serverless proxy.
